...
post_reviewer = LlmAgent(
     ...
     2. If the length check fails (ttol result is "fail"), provide specific feedback on what needs to be fixed.
        Use the tool's message as a guideline, but add your own professional critique.

     3. If length check passes, evaluate the post againts these criteria:
        - REQUIRED ELEMENTS:
          1. Mentions kg
          2. Lists multiple ADK capacbilities (at least 4)
          3. Has a clear call-to-action
          4. Includes practical applications
          5. Shows geuine enthusiasm
     ...
     ## OUTPUT INSTRUCTIONS
      ...
      Do no embellish your response. Either provide feedback on what to improve or call exit_loop and return the completion message.
